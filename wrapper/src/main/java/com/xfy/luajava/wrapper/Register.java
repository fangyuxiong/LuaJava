...
public class Register {
    ...
    private static final List<UDHolder> udHolders = new ArrayList<>(20);
    private static final List<SHolder> sHolders = new ArrayList<>(20);
    private static final List<StringEnumHolder> seHolders = new ArrayList<>(10);
    private static final List<NumberEnumHolder> neHolders = new ArrayList<>(10);
    private static final Map<Class, Class<? extends LuaUserdata>> udClassMap = new HashMap<>(20);
    private static final List<SIHolder> siHolders = new ArrayList<>(10);

    private static final HashMap<Class, String> luaClassNameMap = new HashMap<>();
    private static final AllUserdataHolder allUserdataHolder = new AllUserdataHolder();

    public static boolean useAllUserdataHolder = true;
     ...
     * <p>
     * 在lua中，只有一个名称对应相关类
     * 类中必须含有{@link com.xfy.annotation.LuaClass} 和 {@link com.xfy.annotation.LuaBridge}注解
     * 编译器将通过lprocess库生成相关代码
     *
     * @see #registerUserdata(Class, boolean, String...)
     */
    public static void registerUserdata(String luaClassName, boolean lazy, Class clz) throws RegisterError {
     ...
     */
    public static void registerUserdata(Class clz, boolean lazy, String... luaClassName) throws RegisterError {
     ...
     */
    public static void registerUserdata(UDHolder holder) {
        if (useAllUserdataHolder)
            allUserdataHolder.add(holder);
        else
            udHolders.add(holder);
    }
    ...
    /**
     * 注册静态bridge
     * class中必须含有{@link com.xfy.annotation.LuaClass} 和 {@link com.xfy.annotation.LuaBridge}注解
     * 编译器将通过lprocess库生成相关代码
     */
    public static void registerStaticBridge(String luaClassName, Class clz) {
     ...
     */
    public static void registerStaticBridge(SHolder holder) {
        sHolders.add(holder);
    }
     ...
     * @param clz          java类
     */
    public static void registerSingleInstance(String luaClassName, Class clz) throws RegisterError {
     ...
     */
    public static void registerSingleInstance(UDHolder holder, String key) {
     ...
     */
    public static void registerSingleInstance(SIHolder h) {
        siHolders.add(h);
    }
     ...
     */
    public static void registerEnum(Class clz) {
        ConstantClass cc = (ConstantClass) clz.getAnnotation(ConstantClass.class);
        if (cc == null) {
            throw new RegisterError("register enum failed! class must have a ConstantClass annotation");
        }
        ...
        throw new RegisterError("constant type must be number type or String");
    }
     ...
     * 注册字符串型枚举
     */
    public static void registerEnum(String luaClassName, String[] keys, String[] values) {
     ...
     * 注册数字型枚举
     */
    public static void registerEnum(String luaClassName, String[] keys, double[] values) {
    ...
    public static void install(Globals g) {
        if (!useAllUserdataHolder) {
            for (UDHolder h : udHolders) {
                if (h.lazy) {
                    g.registerLazyUserdataSimple(h.luaClassName, h.clz, h.methods);
                } else {
                    g.registerUserdataSimple(h.luaClassName, h.clz, h.methods);
                }
            }
        }
        ...
        for (SHolder h : sHolders) {
            g.registerStaticBridgeSimple(h.luaClassName, h.clz, h.methods);
        }
        end = System.nanoTime();
     ...
     * 将所有userdata放到一起同时注册
     */
    private static final class AllUserdataHolder {
        final int INIT = 50;
        ...
        void add(UDHolder h) {
            ...
            lpcns.add(Globals.findLuaParentClass(h.clz, luaClassNameMap));
        ...
        private static int[] set(int[] arr, int index, int value) {
            if (arr.length > index) {
                arr[index] = value;
                return arr;
            }
        ...
        private static boolean[] set(boolean[] arr, int index, boolean value) {
            if (arr.length > index) {
                arr[index] = value;
                return arr;
            }
        ...
        private static int[] get(int[] arr, int len) {
            if (arr.length == len)
                return arr;
            return Arrays.copyOf(arr, len);
        }

        private static boolean[] get(boolean[] arr, int len) {
            if (arr.length == len)
                return arr;
            return Arrays.copyOf(arr, len);
        }
     ...
     */
    public static void createSingleInstance(Globals g) {
    ...
    public static final class UDHolder {
         ...
         */
        private Class<? extends LuaUserdata> clz;
        /**
         * 所有方法名
         */
