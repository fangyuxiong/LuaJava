...
import com.xfy.annotation.CreatedByApt;
...
class Generator {
    ...
    private static final String PACKAGE_NAME = "com.xfy.luajava";
    ...
    private static final TypeName UserdataTranslator = ClassName.get(PACKAGE_NAME + ".wrapper", "Translator");
    ...
    private void addDefaultMethodsForUD(TypeSpec.Builder builder) {
        ...
        mb = MethodSpec.methodBuilder("__onLuaGc")
                ...
                .returns(void.class);
        if (__onLuaGc != null) {
            mb.addStatement("if (javaUserdata != null) (($T)javaUserdata).__onLuaGc()", typeElement);
        } else if (parent != null) {
            mb.addStatement("super.__onLuaGc()");
        }
        mb.addStatement("javaUserdata = null");
        builder.addMethod(mb.build());
    }
    ...
    private void addMethodImpl(MethodSpec.Builder mb, ExecutableElement e, boolean sb) {
        ...
        if (plen == 0) {
            ...
            directlyCall(mb, call, e.getSimpleName(), returnType);
            return;
        }
        // 函数参数为LuaValue[] 特殊处理
        ...
        for (int i = 0; i < plen; i++) {
            ...
            if (Globals.equals(vt)) {
                ...
                if (isBoolean(uvt)) {
                    ...
                    mb.addStatement("($T) ((p.length > $L && p[$L].isNumber()) ? p[$L].toDouble() : null)", vt, index, index);
                }
            } else if (isString(vt)) {
        ...
        returnExceptVoidAndLuaValue(mb, "p" + plen, returnType);
    }

    private static void returnExceptVoidAndLuaValue(MethodSpec.Builder mb, final String call, TypeName returnType) {
        mb.addCode("return ");
    ...
    private void directlyCall(MethodSpec.Builder mb, String call, Name methodName, TypeName returnType) {
        if (isVoid(returnType)) {
        ...
        returnExceptVoidAndLuaValue(mb, call, returnType);
    }
