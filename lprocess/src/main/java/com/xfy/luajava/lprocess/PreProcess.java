...
import com.xfy.annotation.BridgeType;
...
class PreProcess {
    ...
    private static Generator parse(Map<TypeElement, Generator> all, TypeElement element) {
        ...
        for (int i = 0; i < len; i ++) {
            ...
            if (LUA_GC_NAME.equals(e.getSimpleName().toString())) {
                builder.setLuaGc(e);
                continue;
            }
            if (SET_UD_NAME.equals(e.getSimpleName().toString())) {
                builder.setSettingUD(e);
                continue;
            }
            if (LUA_EQ_NAME.equals(e.getSimpleName().toString())) {
                builder.setLuaEq(e);
                continue;
            }
            if (bridge == null)
            ...
            if (type == BridgeType.SETTER || type == BridgeType.GETTER) {
                ...
                if (e2 == null) {
                    error(e, "cannot find %s method or property for %s", otherName, e.getSimpleName().toString());
                    return null;
                }
                skipI++;
