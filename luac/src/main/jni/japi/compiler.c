...
static inline void throwUndumpError(JNIEnv *env, const char *msg)
{
    ...
    if (!UndumpError)
        UndumpError = GLOBAL(env, (*env)->FindClass(env, JAVA_PATH "exception/UndumpError"));
...
static int return_success(lua_State *L, char *filename, int autosave)
{
    ...
    #if defined(J_API_INFO)
    m_malloc(filename, (strlen(filename) + 1 + isBinaryPath(filename)) * sizeof(char), 0);
    #else
    free(filename);
    #endif
...
int searcher_Lua(lua_State *L)
{
    ...
    if (real_loadfile(L, filename, name))
    {
        ...
        if (isbin)
        {
            ...
            remove(filename);
            #if defined(J_API_INFO)
            m_malloc(filename, (strlen(filename) + 2) * sizeof(char), 0);
            #else
            free(filename);
            #endif
            filename = findfile4lua(L, name);
...
int searcher_java(lua_State *L)
{
    ...
    {
        if (need) detachEnv();
        const char * e = lua_pushfstring(L, "call Globals.____onLuaRequire method return null for module %s", name);
        lua_unlock(L);
        return luaL_error(L, "error loading module '%s' from java :\n\t%s", name, e);
    }
    ...
    else // return file content (byte array ), call real_loadbuffer
    {
        ...
        if (r == LUA_OK)
        {
            if (!isbin)
            {
                ...
                if (bp)
                {
                    bp = lua_pushfstring(L, "%s" LUA_DIRSEP "%s.lua" BINARY_SUFFIX_END, bp, luaL_gsub(L, name, ".", LUA_DIRSEP));
