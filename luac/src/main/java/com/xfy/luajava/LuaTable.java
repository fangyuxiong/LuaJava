...
public class LuaTable extends NLuaValue implements Iterable {
     ...
     */
    private LuaTable(Globals globals, String stackIndex) {
     ...
     */
    @LuaApiUsed
    private LuaTable(long L_state, String stackIndex) {
    ...
    public void set(int index, LuaValue value) {
        ...
        switch (t) {
            ...
            default:    //table function userdata thread
                String nativeKey = value.nativeGlobalKey();
                if (globals.globalsIndex().equals(nativeKey)) {
                    LuaCApi._setTableChild(globals.L_State, nativeGlobalKey, index, value, -t);
                } else {
                    LuaCApi._setTableChild(globals.L_State, nativeGlobalKey, index, nativeKey, t);
                }
                break;
        }
    }
    ...
    public void set(String name, LuaValue value) {
        ...
        switch (t) {
            ...
            default:    //table function userdata thread
                String nativeKey = value.nativeGlobalKey();
                if (nativeKey == null || globals.globalsIndex().equals(nativeKey)) {
                    LuaCApi._setTableChild(globals.L_State, nativeGlobalKey, name, value, -t);
                } else {
                    LuaCApi._setTableChild(globals.L_State, nativeGlobalKey, name, nativeKey, t);
                }
                if (this == globals)
                    value.destroy();
                break;
    ...
    private boolean checkValid() {
        if (!destroyed && !globals.isDestroyed() && nativeGlobalKey != null)
            return true;
        throwNotValid();
        return false;
    }
